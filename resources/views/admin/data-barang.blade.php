...
@section('content')
    ...
    <div class="px-5 py-8 mb-8 bg-white rounded-lg md:w-1/2 xl:w-2/6 w-full shadow-lg">
        ...
        <form action="/barangs" method="POST" enctype="multipart/form-data" class="max-w-sm mx-auto">
            ...
            <div class="mb-5">
    <label for="jumlah_barang" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">
        Jumlah Barang
    </label>
    ...
    @if(session('qr_code'))
    <div class="mb-5 mt-5">
        <label for="qr_code" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">QR Code Barang</label>
        <img src="{{ Storage::url(session('qr_code')) }}" alt="QR Code Barang" class="w-32 h-32">
    </div>
@endif
