<div>
    <form @if ($in_detail) wire:submit.prevent='update' @else wire:submit.prevent='store' @endif
        ...
        <div class="mb-5">
    <label for="jumlah_pinjam" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Jumlah
        Pinjam</label>
    <input type="number" wire:model="jumlah_pinjam" id="jumlah_pinjam"
        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
        placeholder="Jumlah yang ingin dipinjam" required min="1" :max="maxJumlahPinjam" />
</div>
